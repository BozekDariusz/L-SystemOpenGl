...
{
    singleTree.depth=10;
    singleTree.twigColour=(tVector3(0.5f, 0.35f, 0.05f));
    singleTree.leafColour=(tVector3(0.0f, 1.0f, 0.0f));
...
glRotatef(	270, 1, 0,0);
if(false){//true for forest false for single tree
for(int i=0;i<100;i++){
 glPushMatrix();
    for(int j=0;j<10;j++){
   ...
   glPopMatrix();
    ...
    drawTree(singleTree,singleTree.depth);
    glPopMatrix();
